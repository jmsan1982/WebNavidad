...
<table class="table table-striped table-bordered" id="tabla_peliculas">
 ...
 <?php
    $sql = "select p.id,p.titulo, p.duracion, g.genero
        ...
        where p.id = pg.pelicula_id and pg.genero_id = g.id
        ...
        if ($result->num_rows > 0) {//esto es un random de imagenes
           ...
           echo "<tr>";
             $img=rand(0,$tamanyo); 
              echo "<td class='text-center'> <a class='text-warning' href='modificar.php?id=".$row['id']."'/><i class='fas fa-edit'></i>&nbsp;&nbsp;<a class='text-danger' href='eliminar.php?id=".$row['id']."'/><i class='fas fa-trash-alt'></i></td>";//botones para modificar y eliminar peliculas
